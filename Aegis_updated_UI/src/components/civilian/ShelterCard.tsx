...
import { Home, MapPin, Phone, Crosshair, CheckCircle2, XCircle, Sparkles } from 'lucide-react-native';
...
export const ShelterCard: React.FC<Props> = ({ shelter, onNavigate, recommendationReason, isRecommended }) => {
  ...
  return (
    <View style={[styles.card, isRecommended && styles.cardRecommended]}>
      {/* Load Balancer Recommendation Banner */}
      {(isRecommended || recommendationReason) && (
        <View style={[styles.recBanner, isRecommended && styles.recBannerActive]}>
          <Sparkles size={12} color={isRecommended ? '#10B981' : colors.text.secondary} />
          <Text style={[styles.recText, isRecommended && styles.recTextActive]} numberOfLines={1}>
            {recommendationReason || `Recommended: ${shelter.distanceKm} km away • ${occupancyPercent}% occupied`}
          </Text>
        </View>
      )}
      ...
      <View style={styles.header}>
        <View style={styles.titleRow}>
          <View style={[styles.iconBox, { backgroundColor: isRecommended ? 'rgba(16, 185, 129, 0.25)' : 'rgba(16, 185, 129, 0.15)' }]}>
      ...
      {onNavigate && shelter.status === 'OPEN' && (
        <TouchableOpacity
          style={[styles.navButton, isRecommended && styles.navButtonRecommended]}
          onPress={() => onNavigate(shelter)}
          activeOpacity={0.8}
        >
          <Crosshair size={14} color="#FFF" />
          <Text style={styles.navButtonText}>
            {isRecommended ? 'Route to Optimal Shelter' : 'Route to Shelter'}
          </Text>
        </TouchableOpacity>
      )}
    </View>
